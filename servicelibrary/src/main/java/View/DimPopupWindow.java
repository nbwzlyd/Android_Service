 ...
 * 暂时只支持ShowAtLocation的区域外背景变淡，ShowAsDrowDown对应的View以下的区域背景变淡 不要用Dismiss，用Hide
 * 
 * @author zxx09506
 */
public class DimPopupWindow extends PopupWindow {
    ...
    private int mDimColor = R.color.main_black_50;
    ...
    /**
     * Dismiss在Handler.post中执行，如果直接调dismiss，logcat中会有Attempting to destroy the window while
     * drawing!（不是crash）
     */
    ...
    public DimPopupWindow(Context context) {
        ...
        mInAnimation = AnimationUtils.loadAnimation(mContext, R.anim.dimpopup_pop_in);
        mOutAnimation = AnimationUtils.loadAnimation(mContext, R.anim.dimpopup_pop_out);
        ...
        mOutAnimationListener = new AnimationListener() {
            @Override
